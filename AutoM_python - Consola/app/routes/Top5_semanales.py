    ...
    try:
        ...
        for hostname, count in top5.items():
            ...
            if hostname in infra_inv.iloc[:, 0].values:
                ...
                tipo = fila.get("tipo_server") or ""
                ubicacion = fila.get("ubicacion") or ""
                resultado["Microsoft Infra"].append((hostname, tipo, count, detalles_texto, ubicacion))

            elif hostname in ad_inv.iloc[:, 0].values:
                fila = ad_inv[ad_inv.iloc[:, 0] == hostname].iloc[0]
                tipo = fila.get("tipo_server") or ""
                ubicacion = fila.get("ubicacion") or ""
                resultado["Microsoft AD"].append((hostname, tipo, count, detalles_texto, ubicacion))
